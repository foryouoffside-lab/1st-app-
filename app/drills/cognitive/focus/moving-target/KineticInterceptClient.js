...
import {
  Compass, Volume2, VolumeX, Eye, Zap, Ban, RotateCcw, Share2, ArrowLeft
} from 'lucide-react';
  ...
  return (
    <div className="absolute inset-0 z-40 flex" style={{ background: 'rgba(5,5,8,0.97)' }}>
      ...
      <div className="flex-1 flex flex-col justify-center gap-3 px-6 sm:px-8 py-4 min-w-0">
        {/* The three tiles are identical in every drill: Best Score, Accuracy, XP.
            Best Score is the one that carries the solo loop now — there is no
            public solo leaderboard, so "beat your own number" is the target.
            Combo moved out: an endurance run inflates it without bound, which
            makes it useless for comparing one run against the next. */}
        <div className="grid grid-cols-3 gap-2">
          <ResultStat label="Best Score" value={(summary.bestScore ?? 0).toLocaleString()} color="text-yellow-400" />
        ...
        <div className="flex gap-2">
          <button onClick={onPlayAgain} className="flex-1 py-3 rounded-[13px] bg-gradient-to-r from-red-500 to-orange-600 text-white font-extrabold text-xs uppercase tracking-wider cursor-pointer">
            Play Again
          </button>
