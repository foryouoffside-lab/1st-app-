...
// ============================================================
const NEW_STORAGE_KEY = 'skilldrills_reflex_training_drill_v1';
...
const getSavedData = () => {
  try {
    ...
    const legacyScore = localStorage.getItem('skilldrills_stop-and-go-dash_best');
    const legacyCombo = localStorage.getItem('skilldrills_stop-and-go-dash_best_combo');
    const legacyLvl = localStorage.getItem('skilldrills_stop-and-go-dash_best_lvl');
    const legacySessions = localStorage.getItem('skilldrills_stop-and-go-dash_sessions');
...
export default function ReflexTrainingDrillClient() {
  ...
  const endGame = useCallback(async () => {
    ...
    const daily = isChallenge
      ...
      : await previewDailyCompletion('reflex-training-drill');
  ...
  useEffect(() => {
    ...
    const updateDimensions = () => {
      ...
      if (trackingState.current.currentX === 0) {
        ...
        trackingState.current.idleLimit = (1000 - p * 800) * (0.75 + Math.random() * 0.5);
        trackingState.current.dashLimit = (220 - p * 100) * (0.8 + Math.random() * 0.4);
      }
    };
    ...
    const drawLoop = (ts: number) => {
      ...
      if (trackingState.current.state === 'idle') {
        if (!trackingState.current.idleLimit) {
          trackingState.current.idleLimit = (1000 - p * 800) * (0.75 + Math.random() * 0.5);
        }
        if (trackingState.current.timer > trackingState.current.idleLimit) {
          ...
          trackingState.current.idleLimit = 0;
          trackingState.current.dashLimit = (220 - p * 100) * (0.8 + Math.random() * 0.4);
        ...
        const progress = Math.min(trackingState.current.timer / trackingState.current.dashLimit, 1);
      ...
      if (trackingState.current.lifeTimer > currentLifespanLimit) {
        ...
        trackingState.current.state = 'idle';
        trackingState.current.timer = 99999; // immediate reloc snap re-trigger
        trackingState.current.idleLimit = 1;
        trackingState.current.dashLimit = 0;
        trackingState.current.lifeTimer = 0;
      }
  ...
  }, [phase, targetColor, getTargetRadius, resolveWrong]);
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (dist <= hitRadius) {
      ...
      // Force immediate relocation dash on next frame
      trackingState.current.state = 'idle';
      trackingState.current.timer = 99999;
      trackingState.current.lifeTimer = 0;
      trackingState.current.idleLimit = 1;
      trackingState.current.dashLimit = 0;
  ...
  const shareResult = useCallback(async () => {
    ...
    const url = 'https://skilldrills.online/drills/cognitive/processing-speed/reflex-training-drill';
    ...
    try {
      ...
      const text = `Scored ${endSummary.score} on Reflex Training Drill (${endSummary.accuracy}% accuracy, ${endSummary.bestCombo}x combo) — SkillDrills`;
      if (typeof navigator !== 'undefined' && navigator.share) {
        navigator.share({ title: 'Reflex Training Drill — SkillDrills', text, url }).catch(() => {});
    ...
    return (
      <div className="min-h-[100dvh] flex items-center justify-center bg-[#050508]">
        <div className="text-center">
          ...
          <p className="text-slate-500 font-bold tracking-widest uppercase text-[10px] animate-pulse">Loading Reflex Aim Engine...</p>
        </div>
      </div>
    );
  }
      ...
      >
        ...
        {phase === 'start' && (
          <div className="relative h-full flex items-center justify-center p-5 overflow-y-auto z-40">
            ...
            <div className="relative w-full max-w-[290px] rounded-[20px] border border-white/5 bg-[#0c0c16]/90 backdrop-blur-lg px-5 pt-5 pb-[18px] text-center shadow-[0_16px_40px_rgba(0,0,0,.5)] my-6">
              ...
              <h1 className="text-[17px] font-bold tracking-tight">Reflex Training Drill</h1>

              <div className="flex flex-col gap-1.5 text-left mt-3.5">
                <HowToRow icon={<Eye className="w-3.5 h-3.5 text-cyan-400 flex-shrink-0" />} node={<>Visually acquire the target after each snap relocation</>} />
                <HowToRow icon={<ZapIcon className="w-3.5 h-3.5 text-indigo-400 flex-shrink-0" />} node={<>Click the target in rest or mid-flick before lifespan timeout</>} />
        ...
        {phase === 'countdown' && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-3 bg-black/55 backdrop-blur-[2px]">
            ...
            <span className="text-[10px] text-slate-500">Target snaps at GO</span>
          </div>
        )}
