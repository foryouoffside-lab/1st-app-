  ...
  clockOffsetInFlight = (async () => {
    ...
    try {
      ...
      if (firebase && uid) {
        ...
        if (serverMs) {
          // The server stamped its clock somewhere inside this round trip, so
          // the midpoint of the trip is the best single estimate of the local
          // instant that corresponds to it.
          measured = serverMs - (before + (after - before) / 2);
        }
      }
    } catch (err) {
