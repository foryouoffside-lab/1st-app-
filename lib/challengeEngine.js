...
export const DUEL_DURATION_SECONDS = 45;
...
//
// The Nth duel against the SAME opponent on the same day is worth less rank
// than the first, down to nothing.
//
// What this is actually defending against: two players arranging wins for
// each other. One deliberately loses over and over, the other climbs the
// leaderboard, and nothing about any individual match looks wrong — both
// players are real, both scores are plausible, the duel genuinely happened.
//
// The obvious rule ("no EIQ when duelling someone on your friends list") does
// not work, for two reasons. It is bypassed by simply NOT adding each other
// and queueing for the same drill at the same moment, which in a small player
// pool matches nearly every time; and it does nothing at all about one person
// farming themselves across two accounts, where there is no friendship to
// detect. It also takes the point out of duelling a friend, which is how most
// players in a small pool get a match at all.
//
// ONE-SIDED WINNING is the honest signal — not repetition. That distinction
// is the whole design, and getting it wrong the first time is instructive:
// keying this to matches PLAYED punished the most legitimate thing in the
// Arena. You lose a duel, you hit Rematch, you win it back — and that
// comeback was worth 40% less purely for being a rematch. Two friends who
// trade wins all evening are not colluding, they are playing the game the way
// it is meant to be played.
//
// So the counter tracks how many times this player has BEATEN this specific
// opponent today, and losses never advance it. A player who keeps losing
// rematches is never penalised, and when they finally win it is worth full
// value. Colluders cannot dodge it either: their whole scheme is one person
// taking every win, which is exactly the number this counts. If they even
// out the wins to keep both counters low, the EIQ simply flows back and
// forth between them and nobody climbs — which is the point.
//
// The leading 1s are a free rematch allowance, so an ordinary evening of
// rematches never touches this at all.
//
// SCOPE: only duels where you CHOSE your opponent — a direct invite, a friend
// duel, a rematch, an open-lobby post you picked off the list. Blind
// matchmaking ('Find Duel') is exempt entirely and always pays full, because
// you cannot farm somebody you were not allowed to choose.
//
// That scope is also what closes the obvious hole in the friends-list version
// of this rule. Nothing here ever reads the friends list, so UNFRIENDING
// achieves nothing: an invite is still an invite between two accounts that
// picked each other, friends or not. The only way to leave the decay behind
// is to stop choosing your opponent — at which point you are playing whoever
// the queue hands you, which is not farming.
//
// Applied SYMMETRICALLY, to the loser's loss as much as the winner's gain: a
// fifth rematch is simply not a ranked match for either side. Cutting only
// the winner's gain would leave a losing friend still bleeding full EIQ, and
// would turn repeated duels into a way to grind somebody else's rank down.
//
// HONEST LIMIT: like the rest of the Arena's scoring this runs on the client,
// so a modified client can ignore it. It keeps honest players honest; it is
// not a security boundary, and it is not meant to be one.
// Indexed by how many times you have ALREADY beaten this opponent today.
// The three leading 1s are the free rematch allowance.
const EIQ_REPEAT_FACTORS = [1, 1, 1, 0.6, 0.35, 0.2];

/**
 * Multiplier on the EIQ swing, given how many times the WINNER has already
 * beaten this same opponent today.
 * @param {number} priorWins 0 for the first win of the day over them
 */
export function eiqRepeatFactor(priorWins) {
  const n = Math.max(0, Math.floor(priorWins || 0));
  return n < EIQ_REPEAT_FACTORS.length ? EIQ_REPEAT_FACTORS[n] : 0;
}

/**
 * Local day key for the counters above.
 *
 * Local, not UTC, deliberately: "today" has to mean the player's today, or the
 * curve resets in the middle of someone's evening. It is the same reasoning as
 * todayStr() in lib/dailyChallenge.js.
 */
export function duelDayKey(d = new Date()) {
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

// Guard on the counter map's size. A player realistically duels a handful of
// people a day, but users/{uid} is WORLD-READABLE (every leaderboard fetch
// pulls 50 of these), so an unbounded map here would be paid for by everyone
// else's reads — the same reason photoURL is capped. Past this we stop adding
// new keys rather than let the doc grow; the players already in the map keep
// their decay, and a new opponent simply pays full, which is the safe way to
// be wrong.
const MAX_TRACKED_OPPONENTS = 30;

/**
 * How many times `uid` has already BEATEN `oppUid` today, read from a
 * users/{uid} document. Returns 0 once the stored day rolls over.
 */
export function priorWinsToday(userData, oppUid, today = duelDayKey()) {
  if (!userData || userData.oppDay !== today) return 0;
  return userData.oppWins?.[oppUid] || 0;
}

/**
 * The counter fields to write back for the WINNER of a duel against `oppUid`.
 * Resets the whole map when the day has rolled over, which is what keeps it
 * from growing forever.
 *
 * Only ever called for the winner. A loss must not advance the counter, or
 * losing rematches would quietly devalue the win you are trying to earn —
 * which was the bug this whole shape exists to avoid.
 */
export function nextOpponentWins(userData, oppUid, today = duelDayKey()) {
  const sameDay = userData?.oppDay === today;
  const wins = sameDay ? { ...(userData.oppWins || {}) } : {};
  if (oppUid in wins || Object.keys(wins).length < MAX_TRACKED_OPPONENTS) {
    wins[oppUid] = (wins[oppUid] || 0) + 1;
  }
  return { oppDay: today, oppWins: wins };
}
...
/**
 * Bail out of a match that has been AGREED but has not started playing yet —
 * the lobby, or the 3-2-1 countdown — and tell the other player immediately.
 *
 * This was the single longest dead wait left in the Arena. Leaving at this
 * point wrote nothing at all: forfeitMatch (see DrillWrapper effect 6c) only
 * acts once the match is genuinely 'playing', and there was no other exit
 * path. So the player who stayed had to discover the departure by TIMER:
 *   - still in the lobby -> sat out the full 30s LOBBY_WAIT_SECONDS before
 *     being told the opponent "never loaded in";
 *   - already in the countdown -> nothing bounded them at all. The countdown
 *     ran out, the drill started, and they played a complete 45-second duel
 *     against a player who had already gone, then waited a further 8s
 *     abandoned-match grace before it resolved. Nearly a minute of a match
 *     that was over before it began.
 * Both are now instant: this writes the resolution the moment they leave, and
 * the opponent's live listener puts the "opponent left" screen straight up.
 *
 * Written as status:'declined' rather than a new status because every
 * listener in the app already keys off that one transition. `cancelledBy`
 * carries WHO left, so the screen can say "they left" rather than the
 * inaccurate "they turned this duel down", and so the player who did the
 * leaving is never shown a notice about their own exit.
 *
 * Transactional and narrowly guarded: it only ever moves 'pending'/'accepted'
 * to 'declined'. A match that reached 'playing' is a real duel with EIQ at
 * stake and must go through forfeitMatch instead, and a 'completed' one is
 * history — this can touch neither, so a slow cleanup firing late can never
 * undo a finished result.
 *
 * @returns {Promise<boolean>} true if the match was actually cancelled here.
 */
export async function leaveBeforeStart(challengeId, uid) {
  const firebase = initFirebase();
  if (!firebase || !challengeId || !uid) return false;
  const { db } = firebase;
  const challengeRef = doc(db, 'challenges', challengeId);
  ...
  try {
    return await runTransaction(db, async (tx) => {
      ...
      if (!snap.exists()) return false;
      const data = snap.data();
      if (data.status !== 'accepted' && data.status !== 'pending') return false;
      // Only a participant may cancel a match out from under the other side.
      if (data.fromUid !== uid && data.toUid !== uid) return false;
    ...
    console.error('Failed to cancel match before start:', error);
    return false;
  }
}
...
async function completeMatchInTx(tx, db, challengeId, data, fromScore, toScore, updates, forcedWinnerUid = null) {
  ...
  // How many times these two have already met today, and therefore what this
  // duel is worth. Read from BOTH docs and take the higher: the two counters
  // are written in the same transaction and so normally agree, but if one
  // side's doc was ever reset or failed to write, trusting the lower number
  // would be the direction that hands out free rank. See eiqRepeatFactor.
  // Blind matchmaking is exempt — see the SCOPE note on EIQ_REPEAT_FACTORS.
  // `matchmaking` is stamped on the challenge doc at creation (sendChallenge),
  // so this is decided by how the match was MADE, not by who the players are.
  const chosenOpponent = data.matchmaking !== true;
  const today = duelDayKey();
  // Keyed to the WINNER's prior wins over this opponent: a loss never advances
  // it, so losing rematches can't devalue the win you're chasing.
  const winnerData = winnerUid === data.fromUid ? fromData : toData;
  const loserUidForCount = winnerUid === data.fromUid ? data.toUid : data.fromUid;
  const priorWins = winnerUid === 'draw'
    ? 0
    : priorWinsToday(winnerData, loserUidForCount, today);
  const repeatFactor = chosenOpponent ? eiqRepeatFactor(priorWins) : 1;
  ...
  if (winnerUid !== 'draw') {
    ...
    // The repeat-opponent multiplier lands here, before the forfeit cap, so a
    // rematch that is worth no rank is worth no rank however it ends. Rounded,
    // and NOT floored to 1 — eiqSwing's own floor of 1 exists so a real match
    // always moves the needle, but the whole point of a decayed rematch is
    // that it eventually stops moving it at all.
    const fullSwing = Math.round(
      eiqSwing(data.drillSlug, winnerScore, loserScore, winnerEiqBefore, loserEiqBefore) * repeatFactor
    );
  ...
  // Bump each side's per-opponent counter for today. Written even on a draw
  // (which stakes no EIQ) so that a pair can't reset the curve by arranging
  // draws between the matches they actually care about.
  ...
  // Advance the WINNER's counter only, and only for a chosen-opponent duel.
  // A draw advances nothing — nobody won, and no EIQ moved.
  if (chosenOpponent && winnerUid !== 'draw') {
    if (winnerUid === data.fromUid) {
      Object.assign(fromUpdates, nextOpponentWins(fromData, data.toUid, today));
    } else {
      Object.assign(toUpdates, nextOpponentWins(toData, data.fromUid, today));
    }
  }
  ...
  // So the result screen can explain a reduced (or zero) swing rather than
  // letting it read as a bug. Only stamped when it actually bit.
  if (repeatFactor < 1) {
    updates.eiqRepeatFactor = repeatFactor;
    updates.eiqRepeatWins = priorWins + 1;
  }
...
// How few other people have to be searching before rating-based filtering is
// abandoned entirely and the nearest available opponent is taken at once.
// See the note in scanForMatch: with a pool this small the filter cannot
// improve the match, it can only delay it.
const SMALL_POOL_CANDIDATES = 3;
...
export async function refreshMatchmakingQueue(uid) {
  ...
  const { db } = firebase;
  try {
    await updateDoc(doc(db, 'matchmaking_queue', uid), { createdAt: serverTimestamp() });
  } catch (err) {
    if (err?.code !== 'not-found') console.error('Failed to refresh matchmaking queue entry:', err);
  }
}
...
export async function scanForMatch(user, drillSlug, eiqRange = MATCHMAKING_EIQ_RANGE) {
  ...
  try {
    ...
    // Collect every live candidate first, THEN decide how picky to be. The
    // old shape filtered and ranked in one pass, which meant it could never
    // know whether it had rejected everybody or there had been nobody there
    // at all — two situations that call for opposite behaviour.
    const candidates = [];
    snap.forEach((docSnap) => {
      ...
      candidates.push({
        candidate,
        diff: Math.abs((candidate.eiq || DEFAULT_EIQ) - myEiq),
      });
    });
    if (candidates.length === 0) return null;

    // Nearest by rating, always — ORDERING by EIQ is free and always right.
    candidates.sort((a, b) => a.diff - b.diff);

    // FILTERING by EIQ is what has to adapt to the size of the pool.
    //
    // With only a handful of people searching, a rating window is theatre: it
    // rejects the one available opponent and leaves the player watching a
    // spinner for 30 seconds until the window widens on its own — and then
    // pairs them with that same person anyway. All the wait bought was
    // boredom, which in a small pool is the thing most likely to make someone
    // stop playing altogether.
    //
    // So below SMALL_POOL_CANDIDATES the window is skipped outright and the
    // closest available opponent is taken immediately. Above it there are
    // genuinely alternatives to hold out for, and the time-based widening in
    // matchmakingEiqRange does its job: a fair match first, anyone after 30s.
    //
    // This self-corrects as the game grows — nothing to revisit later. An
    // empty Arena matches instantly; a busy one matches fairly.
    if (candidates.length <= SMALL_POOL_CANDIDATES) return candidates[0].candidate;

    const inRange = candidates.find((c) => c.diff <= eiqRange);
    return inRange ? inRange.candidate : null;
  } catch (err) {
