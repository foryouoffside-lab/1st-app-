...
async function restore(db, uid) {
  ...
  try { await reconcileDrillBests(); } catch { /* start cards only */ }
...
async function backup(db, uid, lastSignatureRef) {
  ...
  try { await Storage.setJSON(OWNER_KEY, { uid, at: new Date().toISOString() }); } catch {}
  return true;
}
 ...
 * @returns {() => void} stop
 */
export function startProgressCloudSync(uid, { onReady = () => {} } = {}) {
  if (!uid || typeof window === 'undefined') return () => {};
  const fb = initFirebase();
  if (!fb || !fb.db) { onReady(false); return () => {}; }
  const db = fb.db;
  ...
  const restoreAndFlush = async () => {
    if (stopped || ready || restoring) return;
    restoring = true;
    // A cached profile can mount the shell before Firebase restores auth.
    // Wait for auth before reading its protected private progress document.
    const result = await (async () => {
      await fb.auth?.authStateReady();
      if (stopped) return { ok: false };
      return restore(db, uid);
    })().catch(() => ({ ok: false }));
    restoring = false;
    if (stopped) return;
    if (!result.ok) {
      onReady(false);
      // A transient auth/network failure must not disable restoration for
      // the rest of the session. Foreground/online events can retry sooner.
      if (retryTimer) clearTimeout(retryTimer);
      retryTimer = setTimeout(restoreAndFlush, 15000);
      return;
    }
    ready = true;
    ...
    onReady(true);
    // Publish readiness even if a concurrent restore already wrote the data.
...
export async function deleteCloudProgress(uid) {
  ...
  const fb = initFirebase();
  if (!fb || !fb.db) return;
  try { await deleteDoc(backupRef(fb.db, uid)); } catch { /* best effort */ }
  try { await Storage.remove(OWNER_KEY); } catch {}
}
