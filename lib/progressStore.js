...
/**
 * "Something worth keeping just changed on this device."
 *
 * Picked up by lib/progressCloud.js, which mirrors XP/streak/bests/weekly
 * badges to the signed-in player's account so a reinstall doesn't wipe their
 * level and rank badge. Sent as a bare window event rather than a direct call
 * so this module — imported by every drill — never pulls the Firestore SDK
 * into a gameplay bundle. No listener (signed out, or on the web) = no-op.
 */
function dispatchProgressChanged() {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new Event('sd:progress-changed'));
}
...
export async function saveDrillResult({
  ...
  // --- Weekly loop ---
  // Finishing today's set (the run that closed it out) counts one day toward
  // the weekly goal. recordWeeklySession is idempotent per local day, so this
  // is safe even though a slow write could let this line run twice for one
  // set. Kept out of the try above so a weekly-store hiccup can't cost the
  // daily 2x bonus.
  let weeklySessionCompleted = false;
  let weeklyBadgeName = null;
  if (dailyChallengeSetComplete) {
    try {
      const { recordWeeklySession } = await import('./weeklyGoal');
      const wk = await recordWeeklySession();
      weeklySessionCompleted = wk.weekJustCompleted;
      weeklyBadgeName = wk.newBadge?.name || null;
    } catch { /* weekly store unavailable — the daily set still counts */ }
    // Push the reminder off "today" now that the session is done — it should
    // next fire tomorrow, not this evening (see lib/dailyReminder.js).
    try {
      const { ensureDailyReminderScheduled } = await import('./dailyReminder');
  ...
  if (dailyXpEligible || streakMilestone || leveledUp || weeklySessionCompleted) {
    dispatchCelebration({
      dailyChallengeCompleted: dailyXpEligible,
      dailyChallengeSetComplete,
      weeklyGoalCompleted: weeklySessionCompleted,
      weeklyBadgeName,
      streakMilestone,
      leveledUp,
      xpEarned: xp,
  ...
  // Daily-session funnel (see lib/analytics.js). Only real, first-time
  // completions — a replay of an already-ticked slot logs nothing, so the
  // "drills completed per session" and "sessions completed" numbers can't be
  // inflated by retries.
  if (dailyXpEligible) {
    logEvent('session_drill_complete', { drill_id: drillId });
  }
  if (dailyChallengeSetComplete) {
    logEvent('daily_session_complete', {});
  }
  if (weeklySessionCompleted) {
    logEvent('weekly_goal_complete', { badge: weeklyBadgeName || 'none' });
  }
