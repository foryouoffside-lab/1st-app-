...
// `hex` mirrors `color` for <canvas>, which can't read Tailwind classes — it's
// what gives the shared score card its per-tier accent (see
// components/shareCardRenderer.js). Keep the two in sync when either changes.
//
// `shareLabel` is the wording used on the *shared image only*, where the
// audience is other people rather than the player. The in-app result screen
// still shows the honest `label`; nobody shares a picture captioned "Needs
// Practice", so the public artifact stays encouraging without touching the
// grade letter, the score, or any scoring maths.
export const GRADE_TIERS = [
  { min: 95, grade: 'S+', label: 'LEGENDARY',      emoji: '🏆', hex: '#facc15', color: 'text-yellow-400',  bg: 'bg-yellow-500/20',  border: 'border-yellow-500/40',  glow: 'shadow-yellow-500/30'  },
  { min: 85, grade: 'S',  label: 'Elite',          emoji: '⚡', hex: '#22d3ee', color: 'text-cyan-400',    bg: 'bg-cyan-500/20',    border: 'border-cyan-500/40',    glow: 'shadow-cyan-500/30'    },
  { min: 75, grade: 'A',  label: 'Excellent',      emoji: '🌟', hex: '#60a5fa', color: 'text-blue-400',    bg: 'bg-blue-500/20',    border: 'border-blue-500/40',    glow: 'shadow-blue-500/30'    },
  { min: 60, grade: 'B',  label: 'Great',          emoji: '💪', hex: '#4ade80', color: 'text-green-400',   bg: 'bg-green-500/20',   border: 'border-green-500/40',   glow: 'shadow-green-500/30'   },
  { min: 45, grade: 'C',  label: 'Good',           emoji: '👍', hex: '#818cf8', color: 'text-indigo-400',  bg: 'bg-indigo-500/20',  border: 'border-indigo-500/40',  glow: 'shadow-indigo-500/30'  },
  { min: 30, grade: 'D',  label: 'Keep Going',     emoji: '📈', hex: '#fb923c', color: 'text-orange-400',  bg: 'bg-orange-500/20',  border: 'border-orange-500/40',  glow: 'shadow-orange-500/30',  shareLabel: 'Building'   },
  { min: 0,  grade: 'F',  label: 'Needs Practice', emoji: '🎯', hex: '#f87171', color: 'text-red-400',     bg: 'bg-red-500/20',     border: 'border-red-500/40',     glow: 'shadow-red-500/30',     shareLabel: 'Warming Up' },
];

/** Look a tier up by its letter — the shared card only receives the letter. */
export function getGradeByLetter(letter) {
  return GRADE_TIERS.find(t => t.grade === letter) || null;
}
...
export function scoreAction({
  ...
  //
  // maxLevel === null means the drill has NO level ceiling (solo endurance
  // runs, where the ramp is the fail condition and so must never stop). The
  // multiplier then grows linearly and without bound instead of being
  // normalised against a top level that doesn't exist. The 0.035 step is
  // chosen so level 15 lands on ×1.49 — within a point of where the old capped
  // formula topped out — so scores stay comparable across the two modes and
  // only genuinely deeper runs pay more.
  const levelMultiplier =
    maxLevel === null
      ? 1 + (Math.max(1, level) - 1) * 0.035
      : maxLevel > 1
        ? 1 + ((Math.min(level, maxLevel) - 1) / (maxLevel - 1)) * 0.5
        : 1;
...
// Exported so the Daily page can advertise the exact bonus it pays. Hard-coding
// the number in the UI is how it ends up promising 50 while the engine awards
// 200 — the two must move together.
export const DAILY_SET_COMPLETE_XP = 200;

export function calcSessionXP({ finalScore, accuracy, isNewBest, firstPlay, dailyChallenge, dailyChallengeSetComplete = false, streakMilestone = null }) {
  const breakdown = [];
  let xp = 0;

  // A run that never scored earns nothing — no base, no first-play, no streak
  // or daily bonus. Guarded HERE rather than at the call sites because all 24
  // drills call this directly to render the "+N XP" tile on the result screen,
  // while progressStore.saveDrillResult calls it again to bank the XP. Gating
  // only one of the two would show the player a number they never received.
  //
  // Note the floor below: baseXP is Math.max(10, …), so without this an idle
  // run paid 10 XP (60 on a drill's first run) while being graded F / 0%.
  if (!(finalScore > 0)) {
  ...
  // Finishing all 3 of today's daily drills is a separate achievement from
  // "double XP on this one" — a flat bonus on top, not itself doubled. It
  // replaces the old training-mission bonus, which paid for the same act
  // (three drills done today) from a card that no longer exists.
