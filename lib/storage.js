...
export const Storage = {
  ...
  /**
   * Every key currently stored, from whichever backend is in use.
   *
   * Exists for clearAllProgress()'s prefix sweep: a full wipe cannot be written
   * as a fixed list of keys, because each drill names its own and those names
   * change (skilldrills_quick_dodge_v2 -> _v3). Returns [] rather than throwing
   * if the backend has no enumeration, so callers can treat it as best-effort.
